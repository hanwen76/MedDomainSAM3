...
class Sam3Image(torch.nn.Module):
    ...
    def _encode_prompt(
        ...
        memory_prompt_embed = torch.zeros(
            (0, *geo_feats.shape[1:]), device=geo_feats.device
        )
        memory_prompt_mask = torch.zeros(
            (*geo_masks.shape[:-1], 0),
            device=geo_masks.device,
            dtype=geo_masks.dtype,
        )
        if self.memory_prompt_builder is not None:
            memory_prompt_embed, memory_prompt_mask = (
                self.memory_prompt_builder.build_prompt(
                    img_feats=img_feats,
                    txt_feats=txt_feats,
                    txt_masks=txt_masks,
                )
        ...
        if encode_text:
            prompt = torch.cat(
                [txt_feats, geo_feats, memory_prompt_embed, visual_prompt_embed], dim=0
            )
            prompt_mask = torch.cat(
                [txt_masks, geo_masks, memory_prompt_mask, visual_prompt_mask], dim=1
            )
        else:
            prompt = torch.cat(
                [geo_feats, memory_prompt_embed, visual_prompt_embed], dim=0
            )
            prompt_mask = torch.cat(
                [geo_masks, memory_prompt_mask, visual_prompt_mask], dim=1
            )
