...
class Sam3TrackerBase(torch.nn.Module):
    def __init__(
        ...
        self.use_static_memory = use_static_memory
        self.static_memory_topk = static_memory_topk
        self.static_memory_text_weight = static_memory_text_weight
        self.static_memory_fusion_alpha = static_memory_fusion_alpha
        self.static_memory_bank = None
        self.static_memory_query_proj = torch.nn.Linear(self.hidden_dim, self.mem_dim)
        self.static_memory_text_prompt = None
        if static_memory_bank_path is not None:
            self.static_memory_bank = StaticMedicalMemoryBank(
    ...
    def set_static_memory_text_prompt(self, text_prompt: str | None):
        self.static_memory_text_prompt = text_prompt

    def _encode_static_memory_text(self, device):
        if self.static_memory_text_prompt is None:
            return None
        if self.backbone is None or getattr(self.backbone, "language_backbone", None) is None:
            return None
        text_outputs = self.backbone.forward_text(
            [self.static_memory_text_prompt],
            device=device,
        )
        language_features = text_outputs["language_features"]
        text_query = language_features.mean(dim=0)
        return text_query

    def _get_static_memory_prompt(self, current_vision_feats, feat_sizes):
        if not self.use_static_memory or self.static_memory_bank is None:
            return None
        if not self.static_memory_bank.is_loaded():
            return None
        ...
        pooled_query = F.adaptive_avg_pool2d(pix_feat, output_size=1).flatten(1)
        pooled_query = self.static_memory_query_proj(pooled_query)
        # Keep the first integration simple and stable for medical batch sizes that are
        # usually small; later we can switch to per-sample retrieval if needed.
        pooled_query = pooled_query.mean(dim=0, keepdim=True)
        text_query = self._encode_static_memory_text(current_vision_feats[-1].device)

        retrieved = self.static_memory_bank.retrieve(
            pooled_query,
            topk=self.static_memory_topk,
            text_query=text_query,
            image_weight=1.0,
            text_weight=self.static_memory_text_weight,
        )
        memory_feats = retrieved.memory_features.to(
            device=current_vision_feats[-1].device,
            dtype=current_vision_feats[-1].dtype,
        )
        memory_pos = retrieved.memory_pos_enc.to(
            device=current_vision_feats[-1].device,
            dtype=current_vision_feats[-1].dtype,
        )

        # Concatenate top-k memories along the prompt sequence dimension, then
        # broadcast the same retrieved bank to each sample in the current batch.
        prompt = memory_feats.flatten(2).permute(0, 2, 1).reshape(-1, 1, self.mem_dim)
        prompt_pos = (
            memory_pos.flatten(2).permute(0, 2, 1).reshape(-1, 1, self.mem_dim)
        )
        prompt = prompt.expand(-1, B, -1)
        prompt_pos = prompt_pos.expand(-1, B, -1)
        return prompt, prompt_pos, retrieved.scores
    ...
    def _prepare_memory_conditioned_features(
        ...
        base_pix_feat = current_vision_feats[-1].permute(1, 2, 0).view(B, C, H, W)
        static_memory_prompt = self._get_static_memory_prompt(
            current_vision_feats=current_vision_feats,
            feat_sizes=feat_sizes,
        )
        if static_memory_prompt is not None:
            prompt, prompt_pos, retrieved_scores = static_memory_prompt
            ...
            score_gate = retrieved_scores.detach().to(
                device=base_pix_feat.device,
                dtype=base_pix_feat.dtype,
            )
            score_gate = score_gate.clamp(min=0.0).mean()
            fusion_gate = self.static_memory_fusion_alpha * score_gate
            memory_delta = pix_feat_with_mem - base_pix_feat
            return base_pix_feat + fusion_gate * memory_delta
